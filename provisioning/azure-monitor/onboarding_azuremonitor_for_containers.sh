...
#
# bash onboarding_azuremonitor_for_containers.sh <azureSubscriptionId> <azureRegionforLogAnalyticsWorkspace> <clusterName> <kubeconfigContextNameOftheCluster> <logAnalyticsWorkspaceResourceId>
# For example:
# bash ./onboarding_azuremonitor_for_containers.sh 00000000-0000-0000-0000-000000000000 eastus myocp42 admin

if [ $# -le 3 ]
then
  echo "Error: This should be invoked with 4 arguments, azureSubscriptionId, azureRegionforLogAnalyticsWorkspace, clusterName and kubeContext name and optional 5th argument logAnalyticsWorkspaceResourceId"
  exit 1
fi
...
echo "logAnalyticsWorkspaceResourceId:"${5}

subscriptionId=${1}
logAnalyticsWorkspaceRegion=${2}
clusterName=${3}
logAnalyticsWorkspaceResourceId=${5}
...
if [ -z $logAnalyticsWorkspaceResourceId ]; then
  ...
  echo "using provided azure log analytics workspace:${logAnalyticsWorkspaceResourceId}"
